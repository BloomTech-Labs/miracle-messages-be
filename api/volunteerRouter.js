...
router.get("/:chapterId", async (req, res) => {
    ...
    try {
      ...
      if (volunteers.length < 1 && leaders.length < 1) { res.status(404).json({ message: "There is no volunteers in this chapter" });}
      else { res.status(200).json({volunteers, leaders}); }
...
router.delete("/:chapterid", authenticationRequired, userInfo, adminCheck, async (req, res) => {
    const chapterId = req.params.chapterid;
    const oktaId = req.body.oktaid;
      try {
       ...
       res.status(200).json({"Message":`${count} volunteer successfully deleted`});
      } catch (error) {
  ...
  router.delete("/:chapterid/leave", authenticationRequired, userInfo, async (req, res) => {
    const chapterId = req.params.chapterid;
    const oktaId = req.userInfo.sub;
      try { 
       ...
       res.status(200).json({"Message":"You have successfully left this chapter"});
        } else {
